  ...
  <div class="roleListBox">
    ...
    <div class="headPortrait">
      <ul ref='ruleUlDom' class="roleUl">
        <!-- 头像框 -->
        <li class="headBox" v-for="(item, index) of role" :key="index" :class="{headBoxActive:index===roleIndex}" @click="chuangeRoleIndex(index)">
          <!-- 头像 -->
          <img :src="item.icon" alt="头像" class="headImg" :class="{headImgActive:index===roleIndex}"/>
          <!-- 姓名标注 -->
          <p class="roleNmae_" :class="{roleNmae_Activ:index===roleIndex}">{{ item.title }}</p>
        </li>
      </ul>
    </div>
    <!-- 右按钮 -->
